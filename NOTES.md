...
5. **Hero search is real** — maps hero tab → browse category (nightlife/accommodation/events-venues), recognises city names (accent-insensitive → city filter), otherwise text search; smooth-scrolls to #browse. Per user: NO visible search button (Enter submits via sr-only button). Date/guest hero fields still cosmetic (availability-filtered search across listings = future work).
6. **City filter** added to browse section (new `cityFilter` in useBrowseStore, wired into ConditionalCuratedCollections too). Deep links: `/?category=&city=&q=#browse`. Per user feedback (same session): NOT a hardcoded dropdown — city PILLS derived from actual listing data with counts ("Douala (5)"), so any new partner city appears automatically; tapping an active pill clears it. City-first results: all matches in the chosen city, then a separated "Aussi disponible dans d'autres villes" block (max 6, best-rated).
